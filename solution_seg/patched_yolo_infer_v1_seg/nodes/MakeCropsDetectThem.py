...
class MakeCropsDetectThem:
    ...
    def __init__(
        ...
        self.max_crops_x = max_crops_x  # Maximum number of crops along the x-axis
        self.max_crops_y = max_crops_y  # Maximum number of crops along the y-axis
        self.rect_mode = rect_mode  # Whether to use rectangle mode
        ...
        self.crops = self.get_crops_xy(
            ...
            max_crops_x=self.max_crops_x,
            max_crops_y=self.max_crops_y,
    ...
    def get_crops_xy(
        ...
        Args:
            ...
            overlap_y (float, optional): Percentage of overlap along the y-axis
                    (how much subsequent crops borrow information from previous ones)
            max_crops_x (int, optional): Maximum number of crops along the x-axis
            max_crops_y (int, optional): Maximum number of crops along the y-axis
            rect_mode (bool): enables the mode to generate crops in a rectangle
            show (bool): enables the mode to display images using plt.imshow

        """    
        image_y, image_x, _ = image_full.shape
        ...
        if show:
            plt.figure(figsize=[x_steps*4, y_steps*4])
    ...
    def _calculate_batch_inference(
        ...
        for pred, crop in zip(predictions, crops):
            ...
            crop.detected_xyxy = pred.boxes.xyxy.cpu().tolist()

            # Get the classes and convert them to a list
            crop.detected_cls = pred.boxes.cls.cpu().tolist()
